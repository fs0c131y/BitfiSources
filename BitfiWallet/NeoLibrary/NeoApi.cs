...
namespace NeoGasLibrary
{
  public static class NeoAPI
  {
    ...
    private static Witness DeserializeWitness(string hex, out int len)
    {
      ...
      var invoLen = (int)hexToLong(hex.Substring(0, 2)) \      
      witness.invocationScript = hex.Substring(4, invoLen);
      var veriLen = (int)hexToLong(hex.Substring(invoLen + 2, 2)) \      witness.verificationScript = hex.Substring(4 + invoLen, veriLen);
    ...
    public static Transaction BuildClamTx(List\\ unclaimed, Net net, string address)
    {
      var tx = new Transaction
      {
        type = 0x02,
    ...
    public static Transaction BuildContractTx(List\\ unspent, Net net, 
    ...
    public static List\\ GetUnclaimed(Net net, string address)
    {
