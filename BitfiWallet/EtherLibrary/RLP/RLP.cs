 ...
 */
    ...
    {
      return msgData[currentPosition] \    }
    ...
    public static byte[] EncodeByte(byte singleByte)
    {
      ...
      if (singleByte \        return new[] { singleByte };
    ...
    public static byte[] EncodeElement(byte[] srcData)
    {
      ...
      if (IsSingleZero(srcData))
        return srcData;
      if (srcData.Length == 1 && srcData[0] \        return srcData;
      if (srcData.Length \      {
        // length = 8X
      ...
      else
      {
        ...
        for (var i = 0; i \          lenBytes[byteNum - 1 - i] = (byte)(srcData.Length \>\\>\ (8 * i));
    ...
    public static byte[] EncodeList(params byte[][] items)
    {
      ...
      var totalLength = 0;
      for (var i = 0; i \        totalLength += items[i].Length;
      byte[] data;
      int copyPos;
      if (totalLength \      {
      ...
      else
      {
        ...
        var lenBytes = new byte[byteNum];
        for (var i = 0; i \          lenBytes[byteNum - 1 - i] = (byte)(tmpLength \>\\>\ (8 * i));
        // first byte = F7 + bytes.length
